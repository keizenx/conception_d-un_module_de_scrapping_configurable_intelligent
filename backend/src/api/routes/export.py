...
def export_text(req: ExportRequest):
    ...
    lines.append(f"Types de champs détectés: {', '.join(summary.get('detected_field_types', []))}")
    lines.append("=" * 80)
    lines.append("")
    ...
    for idx, item in enumerate(items, 1):
        ...
        for field in item.get("fields", []):
            ...
            if field_type == "LINK":
                lines.append(f"  {field_type}: {field.get('text', '')} ({field.get('href', '')})")
            elif field_type == "IMAGE":
                lines.append(f"  {field_type}: {field.get('src', '')} (alt: {field.get('alt', 'N/A')})")
            else:
...
def export_markdown(req: ExportRequest):
    ...
    if not result.get("success"):
        return PlainTextResponse(f"# ERREUR\n\n{result.get('error', 'Erreur inconnue')}")
    ...
    lines.append(f"**URL:** {result.get('url')}")
    lines.append(f"**Total items extraits:** {summary.get('total_items_extracted', 0)}")
    lines.append(f"**Types de champs:** {', '.join(summary.get('detected_field_types', []))}")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    for idx, item in enumerate(items, 1):
        ...
        for field in item.get("fields", []):
            ...
            if field["type"] == "link":
                ...
                lines.append(f"- **{field_type}:** ![{field.get('alt', '')}]({field.get('src', '')})")
            else:
                lines.append(f"- **{field_type}:** {field.get('text', '')}")
        lines.append("")
