...
try:
    response = httpx.post(
        f"{BASE_URL}/analyze",
        json={"url": "https://books.toscrape.com/", "use_js": False, "max_candidates": 1},
        timeout=30.0
    )
...
print("   Note: Playwright doit être installé avec 'pip install playwright' et 'playwright install chromium'")
try:
    response = httpx.post(
        f"{BASE_URL}/analyze",
        json={"url": "https://books.toscrape.com/", "use_js": True, "max_candidates": 1},
        timeout=60.0
    )
    ...
    print(f"   ✗ Timeout - Le serveur n'a pas répondu (Playwright peut prendre plus de temps)")
except Exception as e:
    print(f"   ⚠ Exception: {e}")
